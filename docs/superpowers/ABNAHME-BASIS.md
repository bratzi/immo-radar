...
| A-4 | Kein Objekt fällt still aus dem Radar | **zur Hälfte geschlossen, ZVG offen** — für **Immowelt** bekommt ein Objekt ohne Preis seit `ea8b731` eine `listings`-Zeile ohne `listing_versions`-Zeile statt eines stillen `continue` (Entscheidung des Nutzers vom 2026-09-11). Die Ursache ist vermessen: „Preis auf Anfrage" der Quelle, kein Parserfehler, und die Quote misst die Region (A13). **Noch offen:** (1) der **ZVG**-Fall — `scrapers/zvg-portal/index.ts` verwirft ein Objekt ohne Verkehrswert weiterhin ersatzlos, gemessen drei stehende IDs über acht Läufe (A6), obwohl dieselbe Entscheidung ausdrücklich für beide Quellen galt; (2) der **Produktionsbeleg**, dass die Zeilen wirklich geschrieben werden; (3) der Anspruch gilt bei Immowelt ohnehin nur innerhalb der Rotationsscheibe — preislose Objekte außerhalb fallen in diesem Lauf weiter durch und werden erst später eingeholt |
