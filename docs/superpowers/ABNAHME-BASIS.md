...
| A-4 | Kein Objekt fällt still aus dem Radar | **Code für beide Quellen steht, Produktionsbeleg offen** — für **Immowelt** bekommt ein Objekt ohne Preis seit `ea8b731` eine `listings`-Zeile ohne `listing_versions`-Zeile statt eines stillen `continue` (Entscheidung des Nutzers vom 2026-09-11). Die Ursache ist vermessen: „Preis auf Anfrage" der Quelle, kein Parserfehler, und die Quote misst die Region (A13). Seit dem Merge von `sdd/zvg-a4` (`db0a22e`) gilt dasselbe für **ZVG**: Ein Objekt ohne Verkehrswert bekommt ebenfalls eine `listings`-Zeile statt eines ersatzlosen `continue`; die drei über acht Läufe gemessenen stehenden IDs (A6) wären davon erfasst. **Noch offen:** (1) der **Produktionsbeleg**, dass die Zeilen bei beiden Quellen in einem echten Lauf wirklich geschrieben werden — Lauf `34910160636` (2026-09-14 23:43 UTC, auf `bf0ddf1`) ist dafür ausgelöst, **Auswertung steht aus**; (2) der Anspruch gilt bei Immowelt ohnehin nur innerhalb der Rotationsscheibe — preislose Objekte außerhalb fallen in diesem Lauf weiter durch und werden erst später eingeholt |
